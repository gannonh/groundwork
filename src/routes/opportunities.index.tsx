...
import { CLOSED, evidenceOf, evidenceSubjectOf, filterOf, filterSearch } from '@/components/opportunities/search'
...
const getEvidence = createServerFn({ method: 'GET' })
  .validator((raw: unknown) => {
    ...
    if (typeof input.id !== 'string' || !filter || !mapFilter.success || typeof input.snapshot !== 'string' || input.snapshot === '') {
      throw new Error('Expected a problem or outcome id, an evidence filter, a map filter, and a map snapshot')
    }
    return { subjectId: input.id, filter, mapFilter: mapFilter.data, snapshot: input.snapshot as SnapshotId }
  })
...
export const Route = createFileRoute('/opportunities/')({
  loaderDeps: ({ search }) => ({
    subject: evidenceSubjectOf(search),
    filter: evidenceOf(search),
    mapFilter: filterOf(search),
  }),
  ...
  loader: async ({ deps, parentMatchPromise }): Promise<EvidenceList | null> => {
    if (!deps.filter || !deps.subject) return null
    const { loaderData: map } = await parentMatchPromise
    if (map?.kind !== 'ready') return { kind: 'missing' }
    return getEvidence({
      data: { id: deps.subject, filter: deps.filter, mapFilter: deps.mapFilter, snapshot: map.snapshot },
    })
  },
  shouldReload: false,
