...
- `map-group`: By outcome (`group=outcome`) shows four outcome headers in this order, each with a 12-week sparkline. Cards keep their overall rank number, and their subtitle reads `N accounts · M mentions`. Clicking an outcome's title collapses its group. A header's accounts and ARR numbers open the evidence sheet for the whole outcome, with `outcome=<id>` in the URL and the selection unchanged: Trust the numbers in reports lists 84 accounts, $3.78M ARR. A header with low-confidence mentions shows the amber review pill: Trust the numbers in reports shows 14.
