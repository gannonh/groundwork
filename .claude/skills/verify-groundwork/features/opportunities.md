...
- `e2e/opportunities.spec.ts` covers `map-list`, `map-select`, `map-direct`, `map-low`, `map-unknown`, `map-preset`, `map-filter`, `map-keys`, `map-stack`, a shared URL, and the refetch rule. It does not replace this recipe.
