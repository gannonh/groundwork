...
- `nav-current`: exactly one link has `aria-current="page"`, the one for the current section. Opportunities is current on `/opportunities` with any search params.
...
- `/` redirects to `/opportunities`, which selects the top-ranked card without naming it in the URL. `expect-url` compares the whole path and search, so expect `/opportunities`, not `/`.
